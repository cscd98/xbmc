...
void CRegistry::Bind()
{
  ...
  if (webOSVersion <= 3)
  {
                                        ...
                                        std::uint32_t version)
    {
...

      {
        ...
        if (it != m_singletonBinds.end())
        {
          ...
          CLog::LogF(LOGDEBUG, "webOS: Singleton bind complete for '{}', target valid={}",
                     interface, !!bind.target);
          return;
        }
       }

      {
        ...
        if (it != m_binds.end())
        {
          ...
          CLog::LogF(LOGDEBUG, "webOS: Created proxy for '{}'", interface);
          TryBind(m_registry, target, name, interface, bind.minVersion, bind.maxVersion, version);
          if (target)
          {
            CLog::LogF(LOGDEBUG, "webOS: Binding succeeded, calling addHandler for '{}'", interface);
            m_boundNames.emplace(name, bind);
            bind.addHandler(name, std::move(target));
            CLog::LogF(LOGDEBUG, "webOS: addHandler completed for '{}'", interface);
          }
          else
          {
            CLog::LogF(LOGWARNING, "webOS: Binding failed for '{}', target invalid", interface);
          }
          return;
        }
      }

      CLog::LogF(LOGDEBUG, "webOS: No bind handler for interface '{}'", interface);
    };
    ...
    // Flush to send the get_registry request to the compositor
    CLog::Log(LOGDEBUG, "webOS: Flushing display");
    m_connection.GetDisplay().flush();

    CLog::Log(LOGDEBUG, "webOS: Dispatching pending events");
    ...
    CLog::Log(LOGDEBUG, "webOS: Starting first roundtrip");
    int eventsDispatched = m_connection.GetDisplay().roundtrip();
    CLog::Log(LOGDEBUG, "Wayland connection: First roundtrip complete, {} events dispatched", eventsDispatched);

    CLog::Log(LOGDEBUG, "webOS: Starting second roundtrip for seat capabilities");
    eventsDispatched = m_connection.GetDisplay().roundtrip();
    CLog::Log(LOGDEBUG, "Wayland connection: Second roundtrip complete, {} events dispatched", eventsDispatched);
