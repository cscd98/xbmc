    ...
    const pos, gint channels, AVChannelLayout * layout)
{
  ...
  for (i = 0; i < channels; i++) {
    for (j = 0; j < (gint)G_N_ELEMENTS (_ff_to_gst_layout); j++) {
    ...
    GstAudioChannelPosition * pos)
{
  ...
  if (channel_layout == 0 || channels > 64) {
     ...
     * GStreamer
     */
    if (channels == 1 && channel_layout->nb_channels == AV_CH_LAYOUT_MONO) {
    ...
    const char *fieldname, ...)
{
  ...
  if (context != NULL && context->ch_layout.nb_channels > 0) {
    ...
    if (context->ch_layout.nb_channels > 1 &&
        ...
        gst_audio_channel_positions_to_mask (pos, context->ch_layout.nb_channels, FALSE,
            &mask)) {
    ...
    AVCodecContext * context, gboolean raw)
{
  ...
  gst_structure_get_int (structure, "channels", &context->ch_layout.nb_channels);
...
gst_ffmpeg_audioinfo_to_context (GstAudioInfo * info, AVCodecContext * context)
{
  ...
  context->ch_layout.nb_channels = info->channels;
