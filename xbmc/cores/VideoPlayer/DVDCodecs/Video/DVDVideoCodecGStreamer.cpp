...
namespace
{
constexpr auto VIDEO_SINK = "waylandsink";
...
bool CDVDVideoCodecGStreamer::CreatePipeline(CDVDStreamInfo &hints, CDVDCodecOptions &options) {
  ...
  if(!codec) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() Unable to find ffmpeg codec {}", hints.codec);
    return false;
  }
  ...
  if (!data.pipeline) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - Unable to create pipeline: {}", error->message);
    return false;
  }
  ...
  if(!data.video_sink) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - No videosink");
    return false;
  }

  g_object_set(G_OBJECT(data.video_sink), "display", "/tmp/xdg/wayland-0", nullptr);
  ...
  if (winSystem->SupportsExportedWindow())
  {
    ...
    struct wl_display* wlDisplay = winSystem->GetWaylandConnection()->GetDisplay(); // or .handle() or similar
    ...
    if (GST_IS_VIDEO_OVERLAY(data.video_sink)) {
      CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - video sink is a overlay");
      //CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - exporting window name");
      //gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(data.video_sink), winSystem->GetExportedSurface();
      guintptr surfaceHandle = (guintptr)(winSystem->GetMainSurface());

      if(surfaceHandle) {
        CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - surface handle: {}", surfaceHandle);
        gst_video_overlay_set_window_handle(GST_VIDEO_OVERLAY(data.video_sink), surfaceHandle);
      }
    }
  ...
  if(!data.app_sink) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - No appsink");
    return false;
  }
  ...
  */
  ...
  if(!data.app_source) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::CreatePipeline() - No appsrc");
    return false;
  }
...
bool CDVDVideoCodecGStreamer::AddData(const DemuxPacket &packet)
{
  ...
  if(!m_isReady && pts > 0) {
    CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer::AddData() - pipleline not ready - calculated pts: {}", pts);
  }
...
GstFlowReturn CDVDVideoCodecGStreamer::CBAutoPlugSelect(GstElement *bin, GstPad *pad, GstCaps *caps, GstElementFactory *factory,
  ...
  CLog::Log(LOGDEBUG, "CDVDVideoCodecGStreamer: CBAutoPlugSelect() auto-plugging {}: {}, {} ({})",
  ...
  if(isDecoder) {
    ...
    CLog::Log(LOGERROR, "CDVDVideoCodecGStreamer: CBAutoPlugSelect() decoder found, setting isReady");
